...
function getLastDayNumberOfMonth(year: number, month: number): number {
...
function getLastDayOfMonth(dateStr: string): string {
  ...
  return formatDateOnly(
    parsed.year,
    parsed.month,
    getLastDayNumberOfMonth(parsed.year, parsed.month),
  );
}
...
function generateBatchNumber(
  ...
  const parsed = parseYearMonth(billingMonth);
  if (!parsed) {
    throw new ORPCError("BAD_REQUEST", {
      message: "Invalid billing month",
    });
...
export const paymentBatchesRouter = {
  create: protectedProcedure
    ...
    .handler(async ({ input, context }) => {
      ...
      const billingMonthParts = parseYearMonth(input.billingMonth);
      if (!billingMonthParts) {
        throw new ORPCError("BAD_REQUEST", {
      ...
      const result = await wsDb.transaction(async (tx) => {
        ...
        const billingYear = billingMonthParts.year;
        const billingMonthNum = billingMonthParts.month; // 1-12
        ...
        for (const contractData of activeContracts) {
          ...
          const nextMonthIndex = billingMonthParts.month;
          const nextYear =
            billingMonthParts.year + Math.floor(nextMonthIndex / 12);
          const nextMonth = (nextMonthIndex % 12) + 1;
          updates.nextBillingDate = formatDateOnly(nextYear, nextMonth, 1);
