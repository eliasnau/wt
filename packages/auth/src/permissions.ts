...
export type PermissionAction<R extends PermissionResource> =
  keyof PermissionSpec[R]["actions"] & string;
...
export const permissionList = Object.entries(PERMISSION_SPEC).flatMap(
  ([resource, def]) =>
...
export function getActionLabel<R extends PermissionResource>(
  ...
  const actions = PERMISSION_SPEC[resource].actions as Record<
    string,
    { label: string }
  >;
...
const customStatement = Object.fromEntries(
  Object.entries(PERMISSION_SPEC).map(([resource, def]) => [
    resource,
...
const allCustomActions = Object.fromEntries(
  Object.entries(customStatement).map(([resource, actions]) => [
    resource,
    [...actions],
  ]),
