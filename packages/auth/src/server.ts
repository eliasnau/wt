...
async function getOwnedOrganizationCount(userId: string) {
  const [{ count: organizationCount = 0 } = { count: 0 }] = await db
    .select({ count: count() })
    .from(organizationMember)
    .where(
      and(
  ...
  plugins: [
    ...
    organization({
      ...
      organizationLimit: async (user) => {
        ...
        const limits: Record<typeof userRole, number> = {
          user: 1,
          admin: Number.MAX_SAFE_INTEGER,
        };

        const orgCount = await getOwnedOrganizationCount(user.id);

        return orgCount >= limits[userRole];
      },
