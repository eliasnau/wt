  ...
  if (isLoadingPasskeys) {
    return (
      <Frame className="after:-inset-[5px] after:-z-1 relative flex min-w-0 flex-1 flex-col bg-muted/50 bg-clip-padding shadow-black/5 shadow-sm after:pointer-events-none after:absolute after:rounded-[calc(var(--radius-2xl)+4px)] after:border after:border-border/50 after:bg-clip-padding lg:rounded-2xl lg:border dark:after:bg-background/72">
        <FramePanel>
          <h2 className="font-heading text-xl mb-2 text-foreground">Passkeys</h2>
          <p className="text-sm text-muted-foreground mb-6">
            Use your fingerprint, face, or device PIN for secure passwordless authentication.
          </p>
            ...
            <Tooltip>
              ...
              <TooltipContent>
                <p className="text-xs max-w-xs">
                  Passkeys use your device's biometric authentication (fingerprint, face ID) for secure,
                  passwordless sign-in
                </p>
              </TooltipContent>
            </Tooltip>

            <Button onClick={handleAddPasskey} disabled={isAddingPasskey} variant="outline" size="sm">
              {isAddingPasskey ? (
                <>
            ...
            {passkeys.map((passkey) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <AlertDialog>
                      <AlertDialogTrigger
                        render={<Button variant="ghost" size="icon" className="hover:bg-destructive/10" />}
                      >
      ...
      <Dialog open={!!editingPasskey} onOpenChange={(open) => !open && setEditingPasskey(null)}>
        <DialogPopup>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleUpdatePasskeyName} disabled={isUpdatingName || !newPasskeyName.trim()}>
              {isUpdatingName ? (
                <>
                  <Loader2 className="mr-2 size-4 animate-spin" />
                  Saving...
...
function PasskeyIcon({ aaguid }: { aaguid?: string | null }) {
  ...
  return (
    <Tooltip>
      <TooltipTrigger asChild>
        <div className="p-3 rounded-lg bg-primary/10 cursor-help">
          <Image src={iconUrl} alt={`${name} icon`} width={24} height={24} className="size-6" />
        </div>
      </TooltipTrigger>
      <TooltipContent>
