...
export const MicSelectorItem = (props: MicSelectorItemProps) => {
	...
	const handleSelect = useCallback(() => {
		...
		onOpenChange?.(false);
	}, [onValueChange, onOpenChange, props.value]);
...
async function fetchAudioDevicesWithPermission() {
...
function getErrorMessage(error: unknown) {
	let message = "Failed to get audio devices";
...
export const useAudioDevices = () => {
	...
	const loadDevicesWithoutPermission = useCallback(async () => {
		try {
			setError(null);
			const audioInputs = await fetchAudioDevices();
	...
	const loadDevicesWithPermission = useCallback(async () => {
		...
		try {
			setLoading(true);
			setError(null);
			const audioInputs = await fetchAudioDevicesWithPermission();
			setDevices(audioInputs);
