...
> = {
	/** GeoJSON FeatureCollection data or URL to fetch GeoJSON from */
	data: string | GeoJSON.FeatureCollection<GeoJSON.Point, P>;
	/** Maximum zoom level to show heatmap (default: 12) - transitions to points above this */
	maxZoom?: number;
	/** Minimum zoom level to start showing points (default: 10) */
	pointMinZoom?: number;
	/** Heatmap intensity at low zoom (default: 0.5) */
	intensity?: number;
	/** Heatmap radius in pixels at base zoom (default: 20) */
	radius?: number;
	/** Heatmap opacity (default: 0.8) */
	opacity?: number;
	/** Gradient colors from low to high density (default: yellow to red) */
	colors?: [string, string, string, string, string];
	/** Color for individual points when zoomed in (default: "#ef4444") */
	pointColor?: string;
	/** Property name to use for weighting points (optional) */
	weightProperty?: string;
	/** Callback when an individual point is clicked (only works when zoomed in past maxZoom) */
	onPointClick?: (
		feature: GeoJSON.Feature<GeoJSON.Point, P>,
		coordinates: [number, number],
	) => void;
...
const DEFAULT_HEATMAP_COLORS: [string, string, string, string, string] = [
	"#fff7bc",
	"#fee391",
	"#fec44f",
	"#fe9929",
...
>({
	...
	useEffect(() => {
		...
		// Add point layer (visible when zoomed in)
		map.addLayer({
			id: pointLayerId,
			type: "circle",
			source: sourceId,
	...
	useEffect(() => {
		if (!isLoaded || !map || !onPointClick) return;
	...
	}, [isLoaded, map, pointLayerId, onPointClick]);
